...
double	ft_strtod_base(char *nptr, char **endptr, int base)
{
	double ret = 0;
	int c;
	bool overflow = false;
	...
	{
		if ((ret > HUGE_VAL / base || ret * base > HUGE_VAL - c))
		{
			errno = ERANGE;
			overflow = true;
			ret = HUGE_VAL;
	...
	((nptr[1] == '-' || nptr[1] == '+') && ft_isdigit(nptr[2])))) // exponent
	{
		...
		if (exponent > 0 && (errno == ERANGE || exponent > DBL_MAX_10_EXP || ret > HUGE_VAL / lazy_pow(base == 16 ? 2 : 10, exponent)))
		{
			errno = ERANGE;
			ret = HUGE_VAL;
		}
		else if (exponent < 0 && (errno == ERANGE || exponent < DBL_MIN_10_EXP || ret < DBL_MIN * lazy_pow(base == 16 ? 2 : 10, exponent)))
		{
			errno = ERANGE;
			ret = DBL_MIN;
		}
