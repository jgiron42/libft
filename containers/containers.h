		...
		struct {
			...
			union {
				struct {
					...
					status (*pop_front)(struct s_container *);
					status (*pop_back)(struct s_container *);
				} sequence;
				struct {
					status (*insert)(struct s_container *, data_type);
					status (*erase)(struct s_container *, data_type);
				} associative;
			};
		} container;
	};
